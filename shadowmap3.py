...
if looks_swapped:
    shel["lon"] = shel["위도"]; shel["lat"] = shel["경도"]
else:
    shel["lon"] = shel["경도"]; shel["lat"] = shel["위도"]
...
# 4) PostGIS 저장  ——  느린 to_postgis 대신 COPY 사용
...
def _fast_copy_gdf(gdf: gpd.GeoDataFrame, table_name: str, srid: int = 4326):
    """GeoDataFrame을 WKB HEX로 임시테이블에 COPY 후 타깃 테이블로 적재"""
    import io, psycopg2
...
with engine.begin() as conn:
    ...
    conn.execute(text("""
        ...
        WITH all_shadows AS (
          ...
          FROM shadow_building_20240731_1800
          UNION ALL
          SELECT ST_CollectionExtract(
                   ST_Buffer(
                     ST_MakeValid(
          ...
          FROM shadow_tree_20240731_1800
          UNION ALL
          SELECT ST_CollectionExtract(
                   ST_Buffer(
                     ST_MakeValid(
          ...
          FROM shadow_shelter_20240731_1800
        ),
        nonempty AS (
          SELECT g5179
          FROM all_shadows
    ...
    conn.execute(text("""
        ...
        ANALYZE shadow_union_20240731_1800;
    """))
...
SRC = (127.344776,36.361738)  # (lon, lat)
DST = (127.345180,36.372113)  # (lon, lat) 36.361738, 127.344776
COOL_WEIGHT = 0.8
...
def _fetch_routes(conn_dsn, src, dst):
    ...
    with psycopg2.connect(conninfo) as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        # ── 여기부터 edges_tmp 생성만 교체 ──
        cur.execute("""
            DROP TABLE IF EXISTS edges_tmp;
            CREATE TEMP TABLE edges_tmp AS
            WITH
            params AS (
              SELECT 4.5::float8 AS shade_tol_m,   -- 그늘 완충(버퍼) 폭
                     150::float8 AS near_m,        -- 그늘 근접 필터
                     600::float8 AS corridor_m     -- 코리도어 폭
            ),
            u5179 AS (
              SELECT ST_Subdivide(
            ...
            edges AS (
              SELECT
                ...
                -- 🔁 '길이 기반' 그늘 비율(중심선 ↔ 그늘버퍼 교차 길이 / 중심선 길이)
                LEAST(
                  COALESCE(
                    SUM(
                      ST_Length(
              ...
              FROM ways_raw w              -- ★ 변경: ways_walk → ways_raw
              JOIN corridor c
        ...
        cur.execute("""
            SELECT COUNT(*) AS total,
                   SUM((shade_ratio > 0)::int) AS shaded,
                   AVG(shade_ratio) AS avg_ratio
            FROM edges_tmp;
        """)
        dbg = cur.fetchone()
        print(f"    → edges_tmp: total={dbg['total']}, shaded={dbg['shaded']}, avg={dbg['avg_ratio']}")

        # ── pgr_dijkstra 부분은 그대로 ──
        sql_route = f"""
        WITH
        ok_v AS (
          ...
          JOIN (
            SELECT source AS vid FROM ways_raw   -- ★ 변경: ways_walk → ways_raw
            UNION
            SELECT target AS vid FROM ways_raw   -- ★ 변경: ways_walk → ways_raw
          ) ok ON ok.vid = v.id
        ),
        src AS (
          SELECT id FROM ok_v
